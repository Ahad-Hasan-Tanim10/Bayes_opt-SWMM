...
inp = read_inp_file(r'C:\Users\ATANIM\Documents\Research\Rocky branch\Flood\Calibration_rtc22_1.inp', convert_sections=[SUBCATCHMENTS,SUBAREAS, INFILTRATION, TRANSECTS, DWF])  # type: swmm_api.SwmmInput
#%%
dict = {"fruits": 56}
...
dict
#%%here input file is converted to a dictionary we have to create a function to update the SWMM dictionary
def cn(x,a):
...
#%% import observed discharge at USGS stations Pickens street
...
#%%import observed water level data at USGS stations near Whaley street
...
vf_dis1= vf_dis1.loc["8/25/2022 5:30":"8/25/2022 11:00"]
vf_dep1= vf_dep1.loc["8/25/2022 5:30":"8/25/2022 11:00"]
#%%
...
def dis(mod, obs):
    ...
    #dep= dep.loc["8/25/2022 06:30":"8/25/2022 11:00"]### define the ranage
...
def dep(mod, obs):
    ...
    return r2_depth
#%% define the report start time and stop time in the simulation export
import os
def runf(fn):
    ...
    # using this line below, we turn the 3 columns into a table, called DataFrame using the pandas library. So that we can plot it.
        ...
        df1= df.loc["8/25/2022 5:30":"8/25/2022 11:00"]#report start and end time
        return df1
#%%
...
def dmodelswmmi(a,b,c,d,e,f,g,
    ...
    r = r1+r2
    return r
#%% store the change in a temporary file called 'cal2' and use that input file for iteration purpose
...
#%% define the optimization model and set the model parameters
import sherpa
...
for trial in study:
    ...
    tL.append(now)
    print(now)
#%% sort out the best fit parameters and save the parameters in it
...
from pyswmm.toolkitapi import NodeResults, LinkResults, SimulationTime
f_path = r'C:\Users\ATANIM\Documents\Research\Rocky branch\Flood'
sim_path = os.path.join(f_path, "cal1.inp")
#swmm_model = PySWMM(sim_path)
#swmm_model.swmm_stride(300)
with Simulation(sim_path) as sim:
    S1 = Links(sim)["usgs_dis"]
    S2 = Nodes(sim)["43_depth"]   
  # we need to create a time series table, idx is the time, s1_values, s2_values are the two columns for the runoff of s1 and s1
  ...
  # using this line below, we turn the 3 columns into a table, called DataFrame using the pandas library. So that we can plot it.
    df = pd.DataFrame({'S1 Discharge': s1_values, 'S2 Depth': s2_values}, index=idx)
swmm_m = PySWMM(sim_path)
swmm_m.swmm_open()
swmm_m.swmm_start()
rtc = []
rdd = []
while(True):
    #time =  swmm_m.swmm_stride(1200) # or swmm_stride()
    time =  swmm_m.swmm_stride(1200)
    cur_t = swmm_m.getCurrentSimulationTime()
    st_t = swmm_m.getSimulationDateTime(SimulationTime.StartDateTime)
    rt = swmm_m.getNodeResult('43_depth', NodeResults.newDepth)
    rtc.append(rt)
    if (cur_t-st_t < timedelta(seconds=12000)):
        print(cur_t)
        #break
    elif (cur_t-st_t < timedelta(seconds=36000)):
        print(cur_t)
        time= swmm_m.swmm_stride(1800)
        rd = swmm_m.getLinkResult('usgs_dis', LinkResults.newFlow)
        rdd.append(rd)
    if(cur_t-st_t > timedelta(seconds=36001)):
        break
...
#%% define a SWMM model that can save the simulated results in a dataframe
def dfmswmmi(a,b,c,d,e,f,g,
    ...
    return dfr
#%% 
dfit= dfmswmmi(a[max_id],b[max_id],c[max_id],d[max_id],e[max_id],f[max_id],g[max_id], 
     a1[max_id],b1[max_id],c1[max_id],d1[max_id],e1[max_id],f1[max_id],g1[max_id],
     a2[max_id],b2[max_id],c2[max_id],d2[max_id],e2[max_id],f2[max_id],g2[max_id],
     a3[max_id], b3[max_id], c3[max_id], d3[max_id], e3[max_id], f3[max_id], g3[max_id],
               a4[max_id], b4[max_id], c4[max_id], d4[max_id], e4[max_id], f4[max_id], g4[max_id],
...
plt.plot(dfit['S1 Discharge'])
plt.plot(vf_dis1['discharge']*0.02831 )
plt.show()
fig, axes = plt.subplots(1, 1, figsize=(10, 5), sharey=True)
plt.plot(dfit['S2 Depth'])
plt.plot(vf_dep1['depth']*.3048)
plt.show()
#%%
vf_dis1= vf_dis1.loc["8/24/2022 16:30":"8/25/2022 21:45"]
vf_dep1= vf_dep1.loc["8/24/2022 16:30":"8/25/2022 21:45"]
dfj= pd.concat([dfi,dfi1, dfi2, dfi3, dfi4, dfi5], axis=0, join="inner")
fig, axes = plt.subplots(1, 1, figsize=(10, 5), sharey=True)
plt.plot(dfj['S1 Discharge'])
plt.plot(vf_dis1['discharge']*0.02831 )
plt.show()
fig, axes = plt.subplots(1, 1, figsize=(10, 5), sharey=True)
plt.plot(dfj['S2 Depth'])
plt.plot(vf_dep1['depth']*.3048)
plt.show()
#%%add a new rainfall data in Time series
rsts = inp['TIMESERIES']
#%%
def get_text_lines(file, num_chat_lines):
   with open(file) as src:
       for item in src.split("\n"):
           lines = item.strip()
       return lines[-num_chat_lines: ]
#%%
get_text_lines(rsts, 1)

with open(rsts) as src:
   for item in src.split("\n"):
       print(item.split("\n"))
