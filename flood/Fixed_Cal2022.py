...
df = pd.DataFrame(zipped, columns=['a1', 'b1', 'c1', 'd1', 'e1', 'f1', 'g1',
               ...
               'a5', 'b5', 'c5', 'd5', 'e5', 'f5', 'g5',
               'a6', 'b6', 'c6', 'd6', 'e6', 'f6', 'g6', 'tLx'])
df.to_csv(r'C:\Users\ATANIM\Documents\Research\Rocky branch\Fixed cal\event2022.csv', header = True, index=False)
#%%
df = pd.read_csv(r'C:\Users\ATANIM\Documents\Research\Rocky branch\Fixed cal\event2021.csv')
...
#%% adjust the deviation value with the dataframe
df['a1'] = Nimpl[0]+ df['a1']
df['b1'] = Nimpl[1]+ df['b1']
df['c1'] = Nimpl[2]+ df['c1']
df['d1']= Nimpl[3]+ df['d1']
...
df['a2'] = Npervl[0]+df['a2']
df['b2'] = Npervl[1]+df['b2']
df['c2']= Npervl[2]+df['c2']
df['d2']= Npervl[3]+df['d2'] 
df['e2']= Npervl[4]+df['e2']
df['f2']= Npervl[5]+df['f2']
df['g2']= Npervl[6]+df['g2']
...
#%%
#Get all column names in a data frame in order to fit distribution function to posterior
#steps to get the posterior distributions are
...
for col in df.columns:
    columns.append(col)
# drop the column containing time 
columns = columns[:-1]
#result = {k:[] for k in columns}, an example function for creating dictionary for the column name
#result1 = list(result)
#%%
os.chdir(r"C:\Users\ATANIM\Documents\Research\Rocky branch\Flood")
from Module_name import fitdist
result1 = []
for i in columns:
    result1.append(fitdist(df[i]))
...
for i in range(0,28):
    dis1 = result1[i][0][0]
    distf1.append(dis1)
# Find out what is the best fit distribution for each parameters
col_list = {k:[] for k in columns}
for i, e in zip(col_list, distf1):
    col_list[i] = e
distf2 = []
for i in range(0,28):
    for j in range(0,5):
        dis2 = result1[i][j][0]
        distf2.append(dis2)
dist_ndarr = np.array(distf2).reshape(-1, 5)
distf3 = dist_ndarr.tolist()
print(dist_ndarr)
#%%
# sorting out the best params    
